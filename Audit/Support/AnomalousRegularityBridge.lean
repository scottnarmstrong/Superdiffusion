...
import Audit.Support.AnomalousRegularityCarrierMeasurability

/-!
# Comparator bridge: `Audit.AnomalousRegularity` vocabulary → repository

This file connects the statement-audit vocabulary of
`Audit/AnomalousRegularity/Challenge.lean` (copied verbatim into
`Audit/AnomalousRegularity/SolutionBasic.lean`, which imports only
Mathlib) to the objects of this repository and of the upstream
`Homogenization` package, so that `Audit/AnomalousRegularity/Solution.lean`
is literal gluing.
...
`Audit/Support/GeneratorRenormalizationBridge.lean` is the same file for the other
...
namespace Audit.Support.ARBridge
