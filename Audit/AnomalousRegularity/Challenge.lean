...
certified in `Algsuperdiff/Frozen/Section4/AnomalousRegularity.lean` and
restated in `Algsuperdiff/MainTheorems.lean` as
`Algsuperdiff.anomalous_regularity`.  Content: for every standing ABK model
(diffusivity `nu > 0`, coupling `0 < gamma ≤ 1/4`, `d ≥ 2`, and a shell-sequence
law with independent shells, the exact triadic marginal scaling, (J1)
mean-zero/stationarity/finite range, (J2) the strict Gaussian `C²` cube tail,
(J3) hyperoctahedral and negation invariance, (J4) corrector energy
`cstar · log 3`), every Hölder exponent `0 < alpha ≤ 1 - C sqrt gamma` and every
scale `m`, there is a random minimal depth `X` with an exponential tail below
which every Dirichlet solution on the origin cube of side `3^m` satisfies the
`C^{0,alpha}` excess-decay estimate `3^{(1-alpha)(m-n)}` on centered windows of
scale `n ≤ m`.  See the docstring of `anomalous_regularity` below.
...
(`CoarseGraining`) carriers inlined.  The sole intentional `sorry` is the
...
6. **Sample law.**  Library: a `ProbabilityMeasure` packaging.  Challenge: the
   identical underlying `Measure.comap Subtype.val P` (`cutoffSampleMeasure`);
   that it is a probability measure is a library theorem, not part of the
   statement.
7. **Coefficient field.**  Library:
   `(Cutoff.coefficientCutoff M.nu L omega).toCoeffField`.  Challenge: the same
   function written directly, `x ↦ nu·Id + (∑'_{r : ℕ} j_{L-r})(x)`
   (`coefficientCutoff`).
...
10. **Measurability obligations.**  `RealizesCstar` and the Koopman construction
   ...
   weaker.  Discharged in `Audit/Support/…CarrierMeasurability.lean`.
