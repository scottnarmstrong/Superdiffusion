...
certified in `Algsuperdiff/Frozen/Section4/GeneratorRenormalization.lean` and
restated in `Algsuperdiff/MainTheorems.lean` as
`Algsuperdiff.generator_renormalization`.  Content: for every standing ABK model
...
`cstar · log 3`) and every scale `m`, a scalar diffusivity `sigmaBarM > 0`
tracking `sqrt (nu² + cstar · gamma⁻¹ · 3^{2 gamma m})` homogenizes the truncated
random Dirichlet problem on the origin cube of side `3^m`, up to a random error
`EB` with a Gaussian-type moment bound, in the uniform norm and in Dirichlet
energy.  See the docstring of `generator_renormalization` below.
...
5. **Sample law.**  Library: a `ProbabilityMeasure` packaging.  Challenge: the
