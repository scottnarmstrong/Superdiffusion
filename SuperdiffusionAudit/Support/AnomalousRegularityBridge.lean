...
import SuperdiffusionAudit.Support.AnomalousRegularityCarrierMeasurability

/-!
# Comparator bridge: `SuperdiffusionAudit.AnomalousRegularity` vocabulary → repository

This file connects the statement-audit vocabulary of
`SuperdiffusionAudit/AnomalousRegularity/Challenge.lean` (copied verbatim into
`SuperdiffusionAudit/AnomalousRegularity/SolutionBasic.lean`, which imports only
Mathlib) to the objects of this repository and of the upstream
`Homogenization` package, so that `SuperdiffusionAudit/AnomalousRegularity/Solution.lean`
is literal gluing.
...
8.  the definitional layer, named, and the full stream field, its carrier
    and its law.

`SuperdiffusionAudit/Support/GeneratorRenormalizationBridge.lean` is the same file for the other
challenge's vocabulary; the two differ only in the import, the namespace, the
opened vocabulary namespace, the last declaration of §14, and the vocabulary
namespace named in the first declaration of §15.
-/

namespace SuperdiffusionAudit.Support.ARBridge

open Algsuperdiff.StatementAudit.AnomalousRegularity
open MeasureTheory
...
theorem normalizedVolumeMeasureOn_eq (A : Set (Vec d)) :
    normalizedVolumeMeasureOn A =
      Algsuperdiff.Section4.Support.normalizedVolumeMeasureOn A := rfl


/-! ## 15. The full stream field, its carrier and its law, named
...
theorem translate_eq (z : Vec d) :
    Algsuperdiff.StatementAudit.AnomalousRegularity.ShellField.translate z =
...
end SuperdiffusionAudit.Support.ARBridge
