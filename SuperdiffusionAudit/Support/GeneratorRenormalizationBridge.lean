...
import SuperdiffusionAudit.Support.GeneratorRenormalizationCarrierMeasurability

/-!
# Comparator bridge: `SuperdiffusionAudit.GeneratorRenormalization` vocabulary → repository

This file connects the statement-audit vocabulary of
`SuperdiffusionAudit/GeneratorRenormalization/Challenge.lean` (copied verbatim into
`SuperdiffusionAudit/GeneratorRenormalization/SolutionBasic.lean`, which imports only
Mathlib) to the objects of this repository and of the upstream
`Homogenization` package, so that `SuperdiffusionAudit/GeneratorRenormalization/Solution.lean`
is literal gluing.
...
`SuperdiffusionAudit/Support/AnomalousRegularityBridge.lean` is the same file for the other
...
namespace SuperdiffusionAudit.Support.GRBridge
...
theorem translate_eq (z : Vec d) :
    Algsuperdiff.StatementAudit.GeneratorRenormalization.ShellField.translate z =
...
end SuperdiffusionAudit.Support.GRBridge
