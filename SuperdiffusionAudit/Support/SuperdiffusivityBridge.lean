...
import SuperdiffusionAudit.Support.SuperdiffusivityCarrierMeasurability

/-!
# Comparator bridge: `SuperdiffusionAudit.Superdiffusivity` vocabulary → repository

This file connects the statement-audit vocabulary of
`SuperdiffusionAudit/Superdiffusivity/Challenge.lean` (copied verbatim into
`SuperdiffusionAudit/Superdiffusivity/SolutionBasic.lean`, which imports only
Mathlib) to the objects of this repository and of the upstream
`Homogenization` package, so that `SuperdiffusionAudit/Superdiffusivity/Solution.lean`
is literal gluing.
...
6.  the propositional layer: local σ-algebras, triadic cubes, `H¹`/`H¹₀` and
    `Model ↔ ABKModel`;
7.  the definitional layer, named;
8.  the full stream field, its sample carrier, its annealed law and the path
    instances, named, together with the set identity for the exhaustion cubes.

Sections 1--12 are shared with `SuperdiffusionAudit/Support/GeneratorRenormalizationBridge.lean`
and `SuperdiffusionAudit/Support/AnomalousRegularityBridge.lean` (the three challenge
vocabularies agree byte-for-byte there); this file drops the two Dirichlet
sections those need and adds §13.
-/

namespace SuperdiffusionAudit.Support.SDBridge
...
/-! ## 12. The definitional layer, named
...
/-! ## 13. The full stream field, its carrier and its law, named

The whole chain from the shell translation through the full-tail event to the
coefficient field and the annealed law is definitionally shared with the
repository; only the exhaustion cubes need an argument, and they agree as
sets. -/

theorem translate_eq (z : Vec d) :
    Algsuperdiff.StatementAudit.Superdiffusivity.ShellField.translate z =
      Algsuperdiff.Frozen.Assumptions.ShellField.translate z := rfl

theorem localCubeDerivNorm_eq :
    localCubeDerivNorm (d := d) =
      Algsuperdiff.Section3.Provider.Stream.localCubeDerivNorm (d := d) := rfl

theorem localCubeSecondDerivNorm_eq :
    localCubeSecondDerivNorm (d := d) =
      Algsuperdiff.Section3.Provider.Stream.localCubeSecondDerivNorm (d := d) := rfl

theorem shellW1InfGradNorm_eq :
    shellW1InfGradNorm (d := d) =
      Algsuperdiff.Section4.Support.shellW1InfGradNorm (d := d) := rfl

theorem upperGradPartialSum_eq :
    upperGradPartialSum (d := d) =
      Algsuperdiff.Section5.Field.upperGradPartialSum (d := d) := rfl

theorem upperGradBounded_eq :
    UpperGradBounded (d := d) =
      Algsuperdiff.Section5.Field.UpperGradBounded (d := d) := rfl

theorem sharpTailWeight_eq : sharpTailWeight = Algsuperdiff.Section5.Field.sharpTailWeight := rfl

theorem largeCubeDerivGauge_eq :
    largeCubeDerivGauge (d := d) =
      Algsuperdiff.Section3.Provider.Stream.largeCubeDerivGauge (d := d) := rfl

theorem sharpGradientGauge_eq :
    sharpGradientGauge (d := d) =
      Algsuperdiff.Section5.Field.sharpGradientGauge (d := d) := rfl

theorem sharpTailBounded_eq :
    SharpTailBounded (d := d) = Algsuperdiff.Section5.Field.SharpTailBounded (d := d) := rfl

theorem sharpTailGood_eq :
    SharpTailGood (d := d) = Algsuperdiff.Section5.Field.SharpTailGood (d := d) := rfl

theorem ratPoint_eq : ratPoint (d := d) = Algsuperdiff.Section5.Field.ratPoint (d := d) := rfl

theorem fullTailGood_eq :
    FullTailGood (d := d) = Algsuperdiff.Section5.Field.FullTailGood (d := d) := rfl

theorem fullSample_eq (gamma : ℝ) :
    FullSample d gamma = Algsuperdiff.Section5.Field.FullSample d gamma := rfl

theorem fullSampleMeasure_toABKModel (m : Model d) :
    fullSampleMeasure m.gamma m.P =
      (Algsuperdiff.Section5.Field.fullSampleLaw (toABKModel m)).toMeasure := rfl

theorem streamField_eq {gamma : ℝ} :
    streamField (d := d) (gamma := gamma) =
      Algsuperdiff.Section5.Field.streamField (d := d) (gamma := gamma) := rfl

theorem streamCoefficient_eq {gamma : ℝ} :
    streamCoefficient (d := d) (gamma := gamma) =
      Algsuperdiff.Section5.Field.streamCoefficient (d := d) (gamma := gamma) := rfl

theorem intrinsicScale_eq : intrinsicScale = Algsuperdiff.Section5.Support.intrinsicScale := rfl

theorem pathMeasurableSpace_eq (d : ℕ) :
    instPathMeasurableSpace d =
      MarkovProcess.ContinuousPath.instMeasurableSpace (alpha := Homogenization.Vec d) := rfl

theorem pathBorelSpace_eq (d : ℕ) :
    instPathBorelSpace d =
      MarkovProcess.ContinuousPath.instBorelSpace (alpha := Homogenization.Vec d) := rfl

/-- The exhaustion cubes of the challenge are the exhaustion cubes of the
repository.  The repository writes the cube as an axis-parallel product; the
challenge writes its membership condition.  The two are equal as sets, not
definitionally. -/
theorem wholeSpaceCube_eq (d m : ℕ) :
    wholeSpaceCube d m = DivergenceFormProcess.Form.wholeSpaceCube d m := by
  ext x
  simp [wholeSpaceCube, DivergenceFormProcess.Form.mem_wholeSpaceCube_iff]

end SuperdiffusionAudit.Support.SDBridge
