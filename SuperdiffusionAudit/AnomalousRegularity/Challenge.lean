...
/-!
# Anomalous regularity — comparator challenge

Mathlib-only comparator challenge for [ABK] Theorem C (`t.regularity`),
certified in `Algsuperdiff/Frozen/Introduction/AnomalousRegularity.lean` as
`Algsuperdiff.Frozen.Introduction.anomalous_regularity`.  Content: for every
standing ABK model (diffusivity `nu > 0`, coupling `0 < gamma ≤ 1/4`, `d ≥ 2`,
and a shell-sequence law with independent shells, the exact triadic marginal
scaling, (J1) mean-zero/stationarity/finite range, (J2) the strict Gaussian
`C²` cube tail, (J3) hyperoctahedral and negation invariance, (J4) corrector
energy `cstar · log 3`), every Hölder exponent `0 < alpha ≤ 1 - C sqrt gamma`
and every scale `m`, there is a random minimal depth `X` with an exponential
tail below which every Dirichlet solution on the origin cube of side `3^m`
satisfies the `C^{0,alpha}` excess-decay estimate `3^{(1-alpha)(m-n)}` on
centered windows of scale `n ≤ m`.  See the docstring of `anomalous_regularity`
below.

The coefficient field of the statement is the whole stream field of the sample,
`a = nu·Id + k` with `k(x) = ∑_{n ∈ ℤ} (j_n(x) - j_n(0))`, read on the carrier
`FullSample` on which both tails of the scale decomposition converge; no
infrared cutoff enters.  The form with a cutoff `L ≥ m`, uniform in `L` and on
the larger carrier `CutoffSample`, is the more general one and stays part of
the public surface, as `Algsuperdiff.anomalous_regularity` in
`Algsuperdiff/MainTheorems.lean`; the statement here is the one the
introduction prints.

Only definitions needed to read that assertion occur below.  The vocabulary is a
statement-level copy of the repository's, with its upstream
(`CoarseGraining`) carriers inlined; sections 1–10 are shared byte-for-byte
with the Theorem A and Theorem B challenges.  The sole intentional `sorry` is
...
2. **The running diffusivity `sigmaBar`.**  Library: the data terms name the
   canonical `Annealed.sigmaBar M m`, selected by unique choice from its
   infinite-volume characterization.  Challenge: a scalar `sigmaBarM` is
   existentially quantified, carrying the profile display that Theorem B asserts
   for the canonical one — `0 < sigmaBarM` and
   `|sigmaBarM - sqrt (nu² + cstar γ⁻¹ 3^{2γm})| ≤ C √γ |log γ| sigmaBarM`.  The
   challenge is weaker in exactly this respect: it does not pin the scalar to
   the canonical construction, and the witness is formally allowed to depend on
   `alpha` as well as on `m`; the profile display confines it to the paper's own
   Theorem B band.  The library proves the statement for the canonical
   `sigmaBar`.
3. **Model shape.**  Library: `ABKModel`, four nested proposition structures.
   Challenge: `Model`, the identical fields flattened into one structure.
4. **J4 proof arguments.**  The library display evaluates the corrector at two
...
5. **Corrector construction.**  Library: `AddAction`/`VAddInvariantMeasure`
...
6. **Sample law.**  Library: a `ProbabilityMeasure` packaging of the law on the
...
7. **Coefficient field.**  Library: `streamCoefficient M.nu omega`, of the type
...
8. **Zero-shell regular law.**  Library: `ProbabilityMeasure.map` of a
   proved-measurable map.  Challenge: the same measure as `Measure.map` of the
   same function (`zeroShellRegMeasure`).
9. **Shell transformations.**  Library: the marginal-scaling, (J1)-stationarity
...
10. **Measurability obligations.**  `RealizesCstar` and the Koopman construction
...
11. **Corrector.**  Library: names the orthogonal-projection corrector.
...
/-- Instance caches for the continuous-linear-map carriers of the shell field's
first and second derivatives.  Under mathlib 4.33 blind instance search does not
reach them through the `Matrix` norm branches; the terms are the ordinary
mathlib ones. -/
instance instNormedAddCommGroupShellDeriv (d : ℕ) :
    NormedAddCommGroup (Vec d →L[ℝ] Mat d) :=
  ContinuousLinearMap.toNormedAddCommGroup
instance instNormedSpaceShellDeriv (d : ℕ) : NormedSpace ℝ (Vec d →L[ℝ] Mat d) :=
  ContinuousLinearMap.toNormedSpace
instance instNormedAddCommGroupShellSecondDeriv (d : ℕ) :
    NormedAddCommGroup (Vec d →L[ℝ] (Vec d →L[ℝ] Mat d)) :=
  ContinuousLinearMap.toNormedAddCommGroup
instance instNormedSpaceShellSecondDeriv (d : ℕ) :
...
def spatialScale (r : ℝ) (j : ShellField d) : ShellField d :=
  ⟨spatialScaleAmbient r j.1, by
    refine ⟨?_, ?_, ?_⟩
    · intro x
      have hscale : HasFDerivAt (fun y : Vec d => r • y)
          (r • ContinuousLinearMap.id ℝ (Vec d)) x := (hasFDerivAt_id x).const_smul r
      have hchain : HasFDerivAt (fun y : Vec d => j (r • y))
          (j.deriv (r • x) ∘SL (r • ContinuousLinearMap.id ℝ (Vec d))) x :=
        (j.hasFDerivAt (r • x)).comp x hscale
      rw [ContinuousLinearMap.comp_smul, ContinuousLinearMap.comp_id] at hchain
      simpa only [spatialScaleAmbient, spatialMap, derivScaleMap,
        ...
        Function.comp_def] using! hchain
    · intro x
      have hscale : HasFDerivAt (fun y : Vec d => r • y)
          (r • ContinuousLinearMap.id ℝ (Vec d)) x := (hasFDerivAt_id x).const_smul r
      have hinner : HasFDerivAt (fun y : Vec d => j.deriv (r • y))
          (j.secondDeriv (r • x) ∘SL (r • ContinuousLinearMap.id ℝ (Vec d))) x :=
        (j.deriv_hasFDerivAt (r • x)).comp x hscale
      have houter : HasFDerivAt (fun y : Vec d => r • j.deriv (r • y))
          ((r • ContinuousLinearMap.id ℝ (Vec d →L[ℝ] Mat d)) ∘SL
            (j.secondDeriv (r • x) ∘SL (r • ContinuousLinearMap.id ℝ (Vec d)))) x :=
        (r • ContinuousLinearMap.id ℝ (Vec d →L[ℝ] Mat d)).hasFDerivAt.comp x hinner
      simpa only [spatialScaleAmbient, spatialMap, derivScaleMap,
...
def translate (z : Vec d) (j : ShellField d) : ShellField d :=
  ⟨translateAmbient z j.1, by
    ...
    · intro x
      have htrans : HasFDerivAt (fun y : Vec d => y + z) (ContinuousLinearMap.id ℝ (Vec d)) x :=
        (hasFDerivAt_id x).add_const z
      simpa only [translateAmbient, translateMap, ContinuousMap.comp_apply,
        Function.comp_def] using! (j.deriv_hasFDerivAt (x + z)).comp x htrans
    · intro x i k
...
/-! ## 13. The theorem -/
/-- **Anomalous regularity ([ABK] Theorem C).**  There is a random minimal
scale with exponential tails below which solutions of the random Dirichlet
problem satisfy the excess-decay estimate, uniformly over the stated
boundary-data classes.  The constants are chosen before the model, depending
only on the dimension and on `cstar`. -/
