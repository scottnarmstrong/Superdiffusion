...
/-!
# Generator renormalization — comparator challenge

Mathlib-only comparator challenge for [ABK] Theorem B (`t.homogenization`),
certified in `Algsuperdiff/Frozen/Introduction/GeneratorRenormalization.lean`
as `Algsuperdiff.Frozen.Introduction.generator_renormalization`.  Content: for
every standing ABK model (diffusivity `nu > 0`, coupling `0 < gamma ≤ 1/4`,
...
corrector energy `cstar · log 3`) and every scale `m`, a scalar diffusivity
`sigmaBarM > 0` tracking `sqrt (nu² + cstar · gamma⁻¹ · 3^{2 gamma m})`
homogenizes the random Dirichlet problem on the origin cube of side `3^m`, up to
a random error `EB` with a Gaussian-type moment bound, in the uniform norm and
in Dirichlet energy.  See the docstring of `generator_renormalization` below.
...
the public surface, as `Algsuperdiff.generator_renormalization` in
...
with the Theorem A and Theorem C challenges.  The sole intentional `sorry` is
...
/-- The volume average `⨍_U f`. -/
noncomputable def volumeAverage (U : Set (Vec d)) (f : Vec d → ℝ) : ℝ :=
...
/-! ## 13. The theorem -/
/-- **Generator renormalization ([ABK] Theorem B).**  For every scale there are a
deterministic coefficient and a random error with the stated moment bound such
that, almost surely, the solution of the random Dirichlet problem and that of the
homogenized problem with the same data are comparable in the uniform norm and
their Dirichlet energies agree, up to the error times the natural norms of the
data.  The constants are chosen before the model, depending only on `d`, `cstar`. -/
