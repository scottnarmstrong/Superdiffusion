...
the disorder (equivalently, the raw moments are bounded by the amplitudes raised to `p`); here `R(t) = ((nu t)^{2-gamma} + cstar gamma⁻¹ t²)^{1/(2(2-gamma))}`
...
a probability law starting at `x`, the full future path law satisfies the
homogeneous Markov restart identity conditional on every past event, and its
transition operators form a strongly continuous Feller family on `C₀`.
The Laplace transform in time of its
...
12. **The process.** Library: the constructed continuous Feller process of each
   sample. Challenge: `IsDiffusionOf` requires a measurable family of path laws,
   the Markov restart identity for every past event and future path event,
   Feller transitions, and the PDE-resolvent identity. The restart identity
   constrains joint temporal dependence, not only one-time distributions.
   The theorem asserts existence of such a full process for every sample.
   Its bounds hold for every law satisfying this characterization.
