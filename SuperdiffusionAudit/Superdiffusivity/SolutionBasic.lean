...
/-- The probability, starting-point and PDE-resolvent conditions on the one-time
marginals. This predicate alone does not specify the temporal dependence. -/
def HasDiffusionMarginals {d : ℕ} (a : Vec d → Mat d)
...
/-- The past of a continuous path through time `s`, generated by its evaluations. -/
def pathHistory {d : ℕ} (s : ℝ≥0) : MeasurableSpace C(ℝ≥0, Vec d) :=
  ⨆ t : Set.Iic s, MeasurableSpace.comap
    (fun path : C(ℝ≥0, Vec d) => path t) inferInstance

/-- Shift a continuous path by a nonnegative time. -/
def pathShift {d : ℕ} (s : ℝ≥0) (path : C(ℝ≥0, Vec d)) : C(ℝ≥0, Vec d) :=
  path.comp ⟨fun t => s + t, continuous_const.add continuous_id⟩

/-- The homogeneous Markov restart identity, tested on every past event and
future path event. It constrains the full temporal law, not only marginals. -/
def HasMarkovRestart {d : ℕ} (Q : Vec d → Measure C(ℝ≥0, Vec d)) : Prop :=
  Measurable Q ∧
    ∀ (x : Vec d) (s : ℝ≥0) (A : Set C(ℝ≥0, Vec d)),
      MeasurableSet[pathHistory s] A →
      ∀ B : Set C(ℝ≥0, Vec d), MeasurableSet B →
        Q x (A ∩ pathShift s ⁻¹' B) = ∫⁻ path in A, Q (path s) B ∂Q x

open scoped ZeroAtInfty in
/-- Transition expectations preserve continuous functions vanishing at infinity
and depend continuously on time in their uniform norm. -/
def HasFellerTransitions {d : ℕ} (Q : Vec d → Measure C(ℝ≥0, Vec d)) : Prop :=
  ∀ f : C₀(Vec d, ℝ), ∃ u : ℝ≥0 → C₀(Vec d, ℝ),
    (∀ t x, u t x = ∫ path, f (path t) ∂Q x) ∧ Continuous u

/-- The non-explosive continuous Feller diffusion of `∇ · a ∇`: its full
path law has the Markov restart property, its transitions are Feller, and its
resolvent is characterized by the cube Dirichlet problems. -/
def IsDiffusionOf {d : ℕ} (a : Vec d → Mat d)
    (Q : Vec d → Measure C(ℝ≥0, Vec d)) : Prop :=
  HasDiffusionMarginals a Q ∧ HasMarkovRestart Q ∧ HasFellerTransitions Q
