...
/-- **Clause (B) of `RootAssemblyConditional.RootLatticeDisplay`, at a free coarse
scale AND off the printed interior gate.**

`RootClauseBCloseChain.rootClauseB_display_interior_freeCoarse` with `z ∈
□_{m-1}` replaced by `(GATE k_c)`. -/
theorem rootClauseB_display_gate_freeCoarse (d : ℕ) [NeZero d] :
    ∃ Cch Cc : ℝ, 0 < Cch ∧ 0 < Cc ∧
  ...
  intro M m0 Ecap hS hgamma L alpha n m kc n' z omega uglob hdat gsrc Khol Kd CB CBc Cdel
    Cosc CdM C1 delta Cest B hgate hmm0 hn' hcore hnm hn'kc hkc hgapTop hsol hgL2 hgW
