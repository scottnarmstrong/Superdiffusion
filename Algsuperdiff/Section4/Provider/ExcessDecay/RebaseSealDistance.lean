...
Authors: Scott
-/
import Algsuperdiff.Section4.Provider.ExcessDecay.RebaseJoinRegated
import Algsuperdiff.Section4.Provider.ExcessDecay.RebaseGeneralClauseRegated

/-!
# The seal distance — the boundary instance, and nothing else

`RebaseJoinRegated` returns the frozen block from the whole general clause at
the `C⁻¹ s⁴` funding line and event; `RebaseGeneralClauseRegated` discharges
...
This module performs the composition, the `(1/2)`-gated script verbatim plus
the two `ε` back-ports: the case split is on the gate, which depends only on
`d, z, n, m` — data bound before the almost-sure quantifier — so it happens
outside the `∀ᵐ` and no measurability question arises.  The gate is still the
`n+2` one: only the two threshold literals move, not the interior clause's
trigger.
...
The hypothesis `hbdry` is the re-gated general clause in real form, restricted to
...
theorem exists_harmonicApproximation_provider_of_boundaryClauseReal (d : ℕ)
    (hbdry : ∃ C : ℝ, 0 < C ∧
    ...
    := by
  classical
  refine exists_harmonicApproximation_provider_of_generalClauseReal_regated d ?_
  obtain ⟨CB, hCB, hB⟩ := hbdry
  obtain ⟨CG, hCG, hG⟩ := exists_generalClauseReal_frontierEmpty_regated d
  have hleB : CB ≤ max CB CG := le_max_left _ _
  have hleG : CG ≤ max CB CG := le_max_right _ _
  have hCpos : (0 : ℝ) < max CB CG := lt_of_lt_of_le hCB hleB
  refine ⟨max CB CG, hCpos, ?_⟩
  intro M s hsrange hregime hfund hs L m n hmL hnm x z hx hz hgeom
  have hep0 : (0 : ℝ) ≤ (max CB CG)⁻¹ * s ^ (4 : ℕ) := clauseEpsilon_nonneg hCpos hs.le
  have hepB : (max CB CG)⁻¹ * s ^ (4 : ℕ) ≤ CB⁻¹ * s ^ (4 : ℕ) :=
    clauseEpsilon_anti hCB hleB hs.le
  have hepG : (max CB CG)⁻¹ * s ^ (4 : ℕ) ≤ CG⁻¹ * s ^ (4 : ℕ) :=
