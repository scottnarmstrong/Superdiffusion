...
/-!
# The provider assembly: the outer join, conditional on the general clause

**This module is a conditional A, not a source-node closure.**  It is the
outer-join theorem at the frozen statement.  The binder is `n + 3 ≤ m` and the
proved interior clause is entered at the weaker `n + 2 ≤ m`; the general
clause's two moving slots read, in `hgen` and in the conclusion alike:

* the general clause's good-event slot `(n+3, z, s/8, 1/2)`;
* the general clause's flux-corrected error index `n+3`.

Its single export takes the anchor's *general* clause in **real** form
as a hypothesis (`hgen`, a caller obligation) and returns the frozen block,
both conjuncts, byte-for-byte under a provider name.  Nothing here proves the
general clause, and nothing here may be read as closing
`l.harmonic.approximation.good.scales`.

What the module *does* discharge, unconditionally:
...
* the **`d = 0` branch**;
* the **binder weakening** `n + 3 ≤ m ⟹ n + 2 ≤ m` at the interior-instance call.

## The caller obligation `hgen`, in words

On the anchor's own binders and on the good event `𝒢(n+3, z; s/8, 1/2)`, almost
surely,
...
```text
  ‖u − v‖_{L̲²(x+□_n)}
      ≤ C ( s^{-4} 𝓔_{s/8}(z+□_{n+3}) ( ‖u − (u)_{W'}‖_{L̲²(W')}
          ...
          + s^{-9/2}           3^{(1+s)n} [∇h]_{H̲^s(W')}
          + s^{-9/2}           3^n        ‖∇h‖_{L̲²(W')} ) ,
      W = (z + □_{n+2}) ∩ □_m ,   W' = (z + □_{n+3}) ∩ □_m ,
```

with every carrier the anchor's own — note the two deliberate
asymmetries: the in-bracket `∇h` average is read at `W`, not `W'`, and the
interior clause's slot, gate and flux index all stay at `n+2`.
...
/-! ## 2. The conditional provider-final -/
...
theorem exists_harmonicApproximation_provider_of_generalClauseReal (d : ℕ)
    (hgen : ∃ C : ℝ, 0 < C ∧
                      ...
                      ENNReal.ofReal
                        ...
                        ENNReal.ofReal
                          ...
                          Algsuperdiff.Section4.Support.normalizedGagliardoESeminormOn
                            ...
                            h.grad +
                        ENNReal.ofReal
                            (C * Real.rpow s (-(9 / 2 : ℝ)) * Real.rpow (3 : ℝ) (n : ℝ)) *
                          MeasureTheory.eLpNorm h.grad 2
                            (Algsuperdiff.Section4.Support.normalizedVolumeMeasureOn
                              (((fun y' => z + y') ''
                                  Homogenization.openCubeSet
